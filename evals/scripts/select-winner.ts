...
function findLatestTournamentReport(): string {
  const files = readdirSync(OUTPUT_DIR)
    .filter(
      (entry) => entry.startsWith("tournament-") && entry.endsWith(".json"),
    )
    .sort();
  if (files.length === 0)
    throw new Error("No tournament report found in evals/output/");
...
function evaluateVariant(
...
): { verdict: "win" | "tie" | "lose"; reason: string } {
  ...
  for (const [suite, scores] of Object.entries(variant.perSuite)) {
    ...
    const variantPct = pct(
      scores.golden.pass,
      scores.golden.total || baseSuite.total,
    );
  ...
  if (baseline.candidateTotal > 0 && variant.candidateTotal > 0) {
    const baselineCandidatePct = pct(
      baseline.candidatePass,
      baseline.candidateTotal,
    );
    const variantCandidatePct = pct(
      variant.candidatePass,
      variant.candidateTotal,
    );
    if (
      baselineCandidatePct - variantCandidatePct >
      CANDIDATE_REGRESSION_THRESHOLD_PP
    ) {
      return {
        verdict: "lose",
    ...
    for (const [suite, scores] of Object.entries(variant.perSuite)) {
      const baseSuite = baseline.perSuite[suite]?.candidates ?? {
        pass: 0,
        total: 0,
      };
      ...
      const baselinePct = pct(baseSuite.pass, baseSuite.total);
      const variantPct = pct(
        scores.candidates.pass,
        scores.candidates.total || baseSuite.total,
      );
...
function pickWinner(baseline: VariantScore, variants: VariantScore[]) {
  ...
  if (winners.length === 0)
    return { winner: undefined as string | undefined, evaluated };
  // Prefer the variant with the most golden wins; tiebreak on candidate pass count.
  winners.sort(
    (a, b) => b.score - a.score || b.candidatePass - a.candidatePass,
  );
...
function promoteWinner(workspace: string) {
  ...
  if (!existsSync(sourceDir))
    throw new Error(`Cannot promote: ${sourceDir} missing`);
  ...
  if (promoted.length === 0) {
    ...
    console.log(
      `Promoted ${promoted.length} file(s) from ${workspace}/ → workspace/: ${promoted.join(", ")}`,
    );
  }
}
...
function main() {
  ...
  if (!baseline)
    throw new Error("Tournament report missing baseline workspace score");
  const variants = report.scores.filter(
    (entry) => entry.workspace !== "workspace",
  );
  ...
  console.log(
    `Baseline: golden ${baseline.goldenPass}/${baseline.goldenTotal}`,
  );
  ...
  const verdictPath = join(
    OUTPUT_DIR,
    `winner-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
  );
