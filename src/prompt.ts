...
export function buildPrompt(
  ...
  if (flowHarnessEnabled) {
    prompt += `\n\n<state_memory_contract>\nAt the start of every user turn, before answering the caller or calling any other tool, call record_turn_understanding exactly once with the structured semantic update for the latest caller message. This is an internal memory update, not a patient-facing action. After it returns, follow the task-plan command packet: phase, missingFacts, nextSafeAction, allowedTools, blockedActions, optional tool/args, and instruction. Never mention record_turn_understanding to the caller.\n</state_memory_contract>`;
  }
...
function buildHarnessOperatingContract(): string {
  return [
    ...
    "At the start of each caller turn, update state with record_turn_understanding, then follow the returned task-plan command: phase, missingFacts, nextSafeAction, allowedTools, blockedActions, optional tool/args, and instruction.",
