...
const COMMON_ADD_PATIENT_FIELDS = {
  ...
  aptSuite: z.string().optional().describe("Apartment or suite number, if any"),
...
function readBackField(routineVision: boolean) {
  return z
    .boolean()
    .optional()
    .describe(
      "Set to true only after reading back the patient's name, date of birth, sex, address, callback phone or inbound caller number, email if provided, insurance, policyholder name, and member ID; " +
        (routineVision
          ? "confirming any provided SSN last four was captured without repeating the digits; "
          : "") +
        "and the caller confirms the details are correct.",
    );
}

const ssnLast4Field = z
  .string()
  .trim()
  .regex(/^\d{4}$/)
  .optional()
...
const addPatientParameters = z
  .object({
    ...COMMON_ADD_PATIENT_FIELDS,
    ssnLast4: ssnLast4Field,
    readBack: readBackField(true),
  })
  .strict();

const medicalOnlyAddPatientParameters = z
  .object({ ...COMMON_ADD_PATIENT_FIELDS, readBack: readBackField(false) })
  .strict();

function createAddPatientTool(
  parameters:
    typeof addPatientParameters | typeof medicalOnlyAddPatientParameters,
  options: { routineVision: boolean },
) {
  return tool({
    name: "add_patient",
    onDuplicate: "reject",
    ...
    execute: async (params, { ctx }) => {
      ...
      if (!checkedInsurance?.accepted || !insurance || !coverageType) {
        return options.routineVision
          ? "Run check_insurance for accepted medical or routine-vision coverage before creating a patient chart."
          : "Run check_insurance for accepted medical coverage before creating a patient chart.";
      }
      ...
      const payload: CreatePatientInput = {
        ...
        ...(coverageType === "routine_vision"
          ? {
              coverageType: "routine_vision",
              ...(!selfPay && "ssnLast4" in params && params.ssnLast4
                ? { ssn: params.ssnLast4 }
                : {}),
            }
          : {}),
...
export const add_patient = createAddPatientTool(addPatientParameters, {
  routineVision: true,
});

export function createMedicalOnlyAddPatientTool() {
  return createAddPatientTool(medicalOnlyAddPatientParameters, {
    routineVision: false,
  });
}
