...
function createUpdateInsuranceTool(coverageTypeDescription: string) {
  return tool({
    ...
    description:
      ...
      `Call this only after check_insurance accepts the new plan for ${coverageTypeDescription}.`,
    parameters: z
      .object({
        insuranceMemberId: z
          .string()
...
export const update_insurance = createUpdateInsuranceTool(
  "the correct medical or routine-vision coverage type",
);

export function createMedicalOnlyUpdateInsuranceTool() {
  return createUpdateInsuranceTool("medical coverage");
}
