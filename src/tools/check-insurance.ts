...
export const check_insurance = tool({
  ...
  description:
    ...
    "A result requiring staff follow-up needs caller permission, then a normal insurance task; transfer only if task creation is unavailable, fails, or the caller declines.",
  parameters: z.object({
    plan: z
      .string()
      .trim()
