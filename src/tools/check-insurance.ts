...
function createCheckInsuranceTool<
  const CoverageTypes extends readonly [
    InsuranceCoverageType,
    ...InsuranceCoverageType[],
  ],
>(coverageTypes: CoverageTypes, coverageTypeDescription: string) {
  return tool({
    ...
    parameters: z.object({
      plan: z
        ...
        .min(1)
        .describe(
          "Plan name exactly as the caller says it or the card shows it",
        ),
      coverageType: z.enum(coverageTypes).describe(coverageTypeDescription),
    }),
...
export const check_insurance = createCheckInsuranceTool(
  ["medical", "routine_vision"],
  "Visit type established by appointment triage; pass medical or routine_vision.",
);

export function createRheumatologyInsuranceTool() {
  return createCheckInsuranceTool(
    ["medical"],
    "Rheumatology visits use medical coverage; pass medical.",
  );
}
