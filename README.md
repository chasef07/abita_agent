...
| TTS | Inworld plugin `inworld-tts-2` | Voice defaults to `Ashley`; 16 kHz PCM |
...
5. **Conversation loop** — AssemblyAI STT → Baseten LLM (with tool calling) → Inworld TTS. The LLM calls tools like `verify_patient`, `get_availability`, `book_appt`, `check_insurance`, `lookup_knowledge`, etc. AdvancedMD-facing tools call the Railway middleware.
...
```bash
pnpm install
cp .env.example .env.local   # fill in LIVEKIT_*, ASSEMBLYAI_*, INWORLD_*, BASETEN_*, AMD_*
...
| `INWORLD_TTS_VOICE` | Optional TTS voice override; defaults to `Ashley` |
